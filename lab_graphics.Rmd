...
points(data[data$X%%2 == 0,], pch=3, cex=1, col="green")
...
Add black text 'Center' at the center of the plot.
...
text(x=center.x, y=center.y, "Center", col="black")
```
...
| Function / Argument | Full Name | Purpose | Example |
|----------------------|-----------|---------|---------|
| `pch` | Plotting Character | Controls symbol type (0–25). | `plot(x, y, pch = 16)` |
| `col` | Color | Color for points/lines. | `plot(x, y, col = "red")` |
| `cex` | Character Expansion | Scales size of points. | `plot(x, y, cex = 1.5)` |
| `main` | Main Title | Title at top of plot. | `plot(x, y, main = "Title")` |
| `sub` | Subtitle | Subtitle under plot. | `plot(x, y, sub = "Source: X")` |
| `xlab` | X Label | Label for x-axis. | `plot(x, y, xlab = "Time")` |
| `ylab` | Y Label | Label for y-axis. | `plot(x, y, ylab = "Value")` |
| `las` | Label Axis Style | Axis label orientation (0–3). | `plot(x, y, las = 1)` |
| `cex.axis` | Character Expansion (Axis) | Scale axis text size. | `plot(x, y, cex.axis = 0.8)` |
| `xlim` | X Limits | Range of x-axis. | `plot(x, y, xlim = c(0,10))` |
| `ylim` | Y Limits | Range of y-axis. | `plot(x, y, ylim = c(0,100))` |
| `type` | Plot Type | `"p"` points, `"l"` lines, `"b"` both, `"h"` vertical lines. | `plot(x, y, type="b")` |
| `bty` | Box Type | Border type: `"o"`, `"n"`, `"l"`, `"7"`, `"c"`, `"u"`. | `plot(x, y, bty="n")` |
| `par(mar)` | Margin | Plot margins (bottom, left, top, right). | `par(mar = c(5,4,2,1))` |
| `par(oma)` | Outer Margin Area | Outer margins around all plots. | `par(oma = c(1,1,1,1))` |
| `par(mfrow)` | Multi-Frame Row | Layout: number of rows/cols for multiple plots. | `par(mfrow = c(2,2))` |
| `layout()` | Layout Manager | Custom plot layout matrix. | `layout(matrix(c(1,2,3,3), 2,2))` |
| `mtext()` | Margin Text | Adds text to plot margins. | `mtext("Outer Text", side = 3, line = 1)` |
| `text()` | Text Annotation | Add text inside plotting region. | `text(x=5, y=10, labels="Here")` |
| `rnorm()` | Random Normal | Random numbers from normal distribution. | `rnorm(100)` |
| `matrix()` | Matrix Constructor | Create matrix from vector. | `matrix(1:6, nrow=2)` |
| `seq()` | Sequence | Generates sequences of numbers. | `seq(1, 10, by = 2)` |
| `rgb()` | Red-Green-Blue Color | Create colors using RGB + alpha. | `rgb(1, 0, 0, 0.5)` |
| `colorRampPalette()` | Color Gradient Function | Creates smooth color gradients. | `cols <- colorRampPalette(c("blue","red"))(20)` |
| `%in%` | Set Membership | Check membership in a vector. | `x %in% c("a","b")` |
| `%--%` | Interval Operator (lubridate) | Create a time interval between two dates/times. | `as.Date("2020-01-01") %--% as.Date("2020-12-31")` |
| `as.Date()` | Convert to Date | Converts strings → Date objects. | `as.Date("2024-05-01")` |
| `dmy()` | Day-Month-Year Parser (lubridate) | Parse dates in `"d-m-y"` format. | `dmy("31-12-2024")` |
