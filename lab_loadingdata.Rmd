...
Download the file book chapter from this [link](../data/common/book_chapter.txt). Read the manual for scan and read the text file named **book_chapter.txt** into R, first as vector and then as a list, with each word in the chapter saved as a entry in the vector or as a single vector in a list.


```{r,accordion=TRUE}
shelley.vec <- scan(file = "./data/common/book_chapter.txt", what = character())
str(shelley.vec)

shelley.list <- scan(file = "./data/common/book_chapter.txt", what = list(character()))
class(shelley.list)
```
...
```{r,accordion=TRUE}
shelley.vec2 <- scan(file = "./data/common/book_chapter.txt", what = 'character', sep = ' ', quote = NULL)
...
In this code block with first import the data from normalized.txt from a file (that you can get [here](../files/normalized.txt)) and accept the defaults for all other arguments in the function. With this settings R will read it as a tab delimited file and will use the first row of the data as colnames (header) and the first column as rownames.

```{r,accordion=TRUE}
expr.At <- read.table("./data/common/normalized.txt")
head(expr.At)
```
...
1.  Download the file [example.data](../files/example.data) to your computer and import it to R using the `read.table()` function. This files consist of gene expression values. Once you have the object in R validate that it looks okay and export it using the `write.table` function.

```{r,accordion=TRUE}
ed <- read.table("./data/common/example.data", sep = ":")
head(ed)
str(ed)
```
...
if(!file.exists("Drosophila_melanogaster.BDGP6.86.gtf.gz") && !file.exists("Drosophila_melanogaster.BDGP6.86.gtf.gz")) download.file("ftp://ftp.ensembl.org/pub/release-86/gtf/drosophila_melanogaster/Drosophila_melanogaster.BDGP6.86.gtf.gz","Drosophila_melanogaster.BDGP6.86.gtf.gz")
if(file.exists("Drosophila_melanogaster.BDGP6.86.gtf.gz")) system("gunzip Drosophila_melanogaster.BDGP6.86.gtf.gz")
```
