---
title: "Working in R environment"
...
```{r, out.width="400pt", fig.align='center', echo=F}
...
```{r pkg.err.ex, eval=T, error=TRUE}
library("XLConnect")
```

In order to install a package from command line, use:
```{r pkg.inst, eval=F}
install.packages('GenABEL', dependencies=T)
```
...
```{r pkg.inst.repo, eval=F}
install.packages('GenABEL', dependencies=T, 
                 repos="http://cran.se.r-project.org")
```
...
```{r pkg.inst.devtools.github, eval=F}
