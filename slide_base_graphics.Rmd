...
# anti-aliasing, a technique that smooths the edges of shapes and text in the plot, 
# making them appear less pixelated or jagged.
...
.pull-left-50[
There are many parameters one can set in `plot()`. Let's have a closer look at some of them:
...
--
* raster (png, jpeg, tiff) or vector graphics (svg, pdf, eps),
...
???
Raster graphics are made of pixels (tiny colored squares). They are resolution-dependent — zooming in makes them blurry. Common formats: PNG, JPEG, TIFF.
Vector graphics are made of mathematical paths (lines, curves, shapes). They are resolution-independent — can be scaled without losing quality. Common formats: SVG, PDF, EPS.

A bonus note about tiff; tiff is a high-quality raster format — great for preserving detail, but still resolution-dependent (unlike vector graphics).
---
...
Other heatmaps with more features:  
- `heatmap.2()` from [´gplots´ package](https://biocorecrg.github.io/CRG_RIntroduction/heatmap-2-function-from-gplots-package).  
- `pheatmap()` from [´pheatmap´ package](https://davetang.github.io/muse/pheatmap.html). 
- `Heatmap()` from [´ComplexHeatmap´ package](https://jokergoo.github.io/ComplexHeatmap-reference/book/). 
