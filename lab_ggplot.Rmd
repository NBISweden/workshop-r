...
<div class="large" style="color:red;">This page is work in progress.</div>
...
```{r}
d <- read.csv("./data/common/covid.csv",header=TRUE,stringsAsFactors=TRUE) %>%
  filter(month<=9) %>%
...
Here is an example of a heatmap showing cases for all European countries. Cases per 1 mil people are shown on the log10 scale.

```{r,fig.height=8}
d2 <- d %>%
  filter(continentExp=="Europe") %>%
  mutate(casesp=round((cases/popData2019)*10^6,3))
...
ggplot(d2,aes(x=date,y=countriesAndTerritories,fill=casesp))+
  geom_tile()+
  labs(x="",y="")+
  scale_fill_gradientn(colors=cols,na.value="grey95",trans="log10",
